...
#  define dmod_vfs_api(VERSION, RET, NAME, PARAMS)                    \
            DMOD_INPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_global_api(VERSION, RET, NAME, PARAMS)             \
            DMOD_GLOBAL_INPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_mal(VERSION, RET, NAME, PARAMS)                    \
            DMOD_MAL_OUTPUT_API(vfs , VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_global_mal(VERSION, RET, NAME, PARAMS)             \
            DMOD_GLOBAL_MAL_OUTPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_api_declaration(VERSION, RET, NAME, PARAMS)        \
            DMOD_INPUT_API_DECLARATION(vfs, VERSION, RET, NAME, PARAMS)
...
#  define dmod_vfs_dif_api_declaration(VERSION, IMPL_MODULE, RET, NAME, PARAMS)  \
            DMOD_DIF_API_DECLARATION(vfs, IMPL_MODULE, VERSION, RET, NAME, PARAMS)
...
#  ifdef DMOD_MAL_vfs
#  define dmod_vfs_mal(VERSION, RET, NAME, PARAMS)            \
                DMOD_MAL_INPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_global_mal(VERSION, RET, NAME, PARAMS)     \
                DMOD_GLOBAL_MAL_INPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#else 
#  define dmod_vfs_mal(VERSION, RET, NAME, PARAMS)            \
                DMOD_MAL_OUTPUT_API(vfs , VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_global_mal(VERSION, RET, NAME, PARAMS)     \
...
#endif
#  define dmod_vfs_api(VERSION, RET, NAME, PARAMS)            \
                DMOD_OUTPUT_API(vfs, VERSION, RET, NAME, PARAMS)
#  define dmod_vfs_global_api(VERSION, RET, NAME, PARAMS)     \
                DMOD_GLOBAL_OUTPUT_API(vfs, VERSION, RET, NAME, PARAMS)
# ifdef ENABLE_DIF_REGISTRATIONS
#  define dmod_vfs_dif(VERSION, RET, NAME, PARAMS)            \
                typedef RET (*dmod_vfs##NAME##_t) PARAMS; \
                const char* const DMOD_MAKE_DIF_SIG_NAME(vfs, NAME) = DMOD_MAKE_DIF_SIGNATURE(vfs, VERSION, NAME);
#  else
#  define dmod_vfs_dif(VERSION, RET, NAME, PARAMS)            \
                typedef RET (*dmod_vfs##NAME##_t) PARAMS; \
                _DMOD_DIF_SIGNATURE_REGISTRATION(DMOD_MAKE_DIF_SIG_NAME(vfs, NAME), DMOD_MAKE_DIF_SIGNATURE(vfs, VERSION, NAME))
#  endif
#  define dmod_vfs_dif_api_declaration(VERSION, IMPL_MODULE, RET, NAME, PARAMS)  \
