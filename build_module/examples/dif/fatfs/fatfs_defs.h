...
#  define dmod_fatfs_api(VERSION, RET, NAME, PARAMS)                    \
            DMOD_INPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_global_api(VERSION, RET, NAME, PARAMS)             \
            DMOD_GLOBAL_INPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_mal(VERSION, RET, NAME, PARAMS)                    \
            DMOD_MAL_OUTPUT_API(fatfs , VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_global_mal(VERSION, RET, NAME, PARAMS)             \
            DMOD_GLOBAL_MAL_OUTPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_api_declaration(VERSION, RET, NAME, PARAMS)        \
            DMOD_INPUT_API_DECLARATION(fatfs, VERSION, RET, NAME, PARAMS)
...
#  define dmod_fatfs_dif_api_declaration(VERSION, IMPL_MODULE, RET, NAME, PARAMS)  \
            DMOD_DIF_API_DECLARATION(fatfs, IMPL_MODULE, VERSION, RET, NAME, PARAMS)
...
#  define dmod_fatfs_mal(VERSION, RET, NAME, PARAMS)            \
                DMOD_MAL_INPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_global_mal(VERSION, RET, NAME, PARAMS)     \
                DMOD_GLOBAL_MAL_INPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#else 
#  define dmod_fatfs_mal(VERSION, RET, NAME, PARAMS)            \
                DMOD_MAL_OUTPUT_API(fatfs , VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_global_mal(VERSION, RET, NAME, PARAMS)     \
...
#endif
#  define dmod_fatfs_api(VERSION, RET, NAME, PARAMS)            \
                DMOD_OUTPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
#  define dmod_fatfs_global_api(VERSION, RET, NAME, PARAMS)     \
                DMOD_GLOBAL_OUTPUT_API(fatfs, VERSION, RET, NAME, PARAMS)
# ifdef ENABLE_DIF_REGISTRATIONS
#  define dmod_fatfs_dif(VERSION, RET, NAME, PARAMS)            \
                typedef RET (*dmod_fatfs##NAME##_t) PARAMS; \
                const char* const DMOD_MAKE_DIF_SIG_NAME(fatfs, NAME) = DMOD_MAKE_DIF_SIGNATURE(fatfs, VERSION, NAME);
#  else
#  define dmod_fatfs_dif(VERSION, RET, NAME, PARAMS)            \
                typedef RET (*dmod_fatfs##NAME##_t) PARAMS; \
                _DMOD_DIF_SIGNATURE_REGISTRATION(DMOD_MAKE_DIF_SIG_NAME(fatfs, NAME), DMOD_MAKE_DIF_SIGNATURE(fatfs, VERSION, NAME))
#  endif
#  define dmod_fatfs_dif_api_declaration(VERSION, IMPL_MODULE, RET, NAME, PARAMS)  \
