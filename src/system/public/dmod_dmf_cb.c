...
void Dmod_Preinit( Dmod_Context_t* Context )
{
    ...
    if( preinit == NULL )
    {
        DMOD_LOG_VERBOSE("Preinit function not set\n");
        return;
    }
    preinit();
}
...
int Dmod_Init( Dmod_Context_t* Context, const Dmod_Config_t* Config )
{
    ...
    if( Dmod_Context_IsValid( Context ) )
    {
        ...
        if( init == NULL )
        {
            DMOD_LOG_VERBOSE("Init function not set\n");
            result = 0;
        }
        else 
        {
...
int Dmod_Deinit( Dmod_Context_t* Context )
{
    ...
    if( Dmod_Context_IsValid( Context ) )
    {
        ...
        if( deinit == NULL )
        {
            DMOD_LOG_VERBOSE("Deinit function not set\n");
            result = 0;
        }
        else 
        {
...
int Dmod_Signal( Dmod_Context_t* Context, int SignalNumber )
{
    ...
    if( Dmod_Context_IsValid( Context ) )
    {
        ...
        if( signal == NULL )
        {
            DMOD_LOG_VERBOSE("Signal function not set\n");
            result = 0;
        }
        else 
        {
